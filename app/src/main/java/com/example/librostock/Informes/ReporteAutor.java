...
public class ReporteAutor {
    ...
    public ReporteAutor(Context contexto) {
    ...
    private void cargarDatos(){
        ...
        String[] encabezadoReporteAutor = {"AUTOR", "TÍTULO", "CANTIDAD"};
        agregarMetadatos("Reporte Autor", "Número Libros x Autor", "LibroStock");
        agregarTitulos("Reporte Libros x Autor", "Número Libros x Autor", fecha, hora);
        //agregarParrafo(textoCorto);
        //agregarParrafo(textLargo);
        ArrayList<String[]> listaReporteAutor = delegar.reporteAutor(contexto);
        ArrayList<ReporteLibrosxAutor> listaReporteLibrosxAutor = delegar.reporteLibrosxAutor(contexto);
        System.out.println("LISTA REPORTE LIBROS x AUTOR: "+listaReporteLibrosxAutor.toString());
        crearTabla(encabezadoReporteAutor, listaReporteLibrosxAutor);
    ...
    private void crearArchivo(){
        ...
        if(!carpeta.exists())
            carpeta.mkdirs();
        archivoPDF = new File(carpeta, "ReporteAutor.pdf");
    }
    ...
    public void crearTabla(String[] encabezado, ArrayList<ReporteLibrosxAutor> listaReporteLibrosxAutor){
        ...
        try{
            ...
            int total = 0;
            for(int indiceFila = 0; indiceFila < listaReporteLibrosxAutor.size(); indiceFila++){
                String autor = listaReporteLibrosxAutor.get(indiceFila).getAutor();
                String cantidad = listaReporteLibrosxAutor.get(indiceFila).getCantidad();
                ArrayList<String> listatitulos = listaReporteLibrosxAutor.get(indiceFila).getTitulos();
            ...
            tablaPie.setWidthPercentage(100);
            PdfPCell celdaPie;
            celdaPie = new PdfPCell(new Phrase("TOTAL LIBROS", LetrasReportes.letraDatosTabla));
            celdaPie.setPaddingLeft(10f);
            celdaPie = new PdfPCell(new Phrase(String.valueOf(total), LetrasReportes.letraDatosTabla));
            celdaPie.setHorizontalAlignment(Element.ALIGN_RIGHT);
            celdaPie.setPaddingRight(10f);
            tablaPie.addCell(celdaPie);
            parrafoPie.add(tablaPie);
    ...
    public void agregarPie(int total){
        try{
            parrafo = new Paragraph();
            PdfPTable tablaPie = new PdfPTable(2);
            tablaPie.setWidths(new int[]{130, 40});
            ...
            celdaPie.setPaddingRight(10f);
            tablaPie.addCell(celdaPie);
            parrafo.add(tablaPie);

            parrafo.setSpacingAfter(30);
            documento.add(parrafo);
        }catch(Exception e){
            Log.e("agregarPie", e.toString());
        }

    }

    public void crearTabla2(String[] encabezado, ArrayList<ReporteLibrosxAutor> listaReporteLibrosxAutor){
        ...
        try{
            ...
            for(int indiceFila = 0; indiceFila < listaReporteLibrosxAutor.size(); indiceFila++){
                ...
                String cantidad = listaReporteLibrosxAutor.get(indiceFila).getCantidad();
                ArrayList<String> listaTitulos = listaReporteLibrosxAutor.get(indiceFila).getTitulos();
                System.out.println("AUTOR: "+autor+" TÍTULOS: "+listaTitulos.toString()+" CANTIDAD: "+cantidad);

                celda = new PdfPCell(new Phrase(autor.toUpperCase(), LetrasReportes.letraDatosTabla));
                celda.setPaddingLeft(10f);
                celda.setVerticalAlignment(Element.ALIGN_MIDDLE);
                celda.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tablaPDF.addCell(celda);
                ...
                celda = new PdfPCell(new Phrase(cantidad, LetrasReportes.letraDatosTabla));
                celda.setPaddingRight(10f);
                celda.setHorizontalAlignment(Element.ALIGN_RIGHT);
                celda.setVerticalAlignment(Element.ALIGN_MIDDLE);
                celda.setBackgroundColor(BaseColor.LIGHT_GRAY);
